...
void initNoises( void )
{
    ...
    // odomNoiseVector6 << 1e-8, 1e-8, 1e-8, 1e-6, 1e-6, 1e-6;
    // odomNoiseVector6 << 1e-7, 1e-7, 1e-7, 1e-5, 1e-5, 1e-5;
...
void loopFindNearKeyframesCloud( pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& submap_size, const int& root_idx)
{
    ...
    // 2024.04.22 yzk zhushi
    /*
    downSizeFilterICP.setInputCloud(nearKeyframes);
    downSizeFilterICP.filter(*cloud_temp);
    */
    // 2024.04.22 yzk zhushi
    // 2024.04.22 yzk shiyong
    pcl::copyPointCloud(*nearKeyframes, *cloud_temp);
    // 2024.04.22 yzk shiyong
    ...
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            */
            ...
            // 2024.04.22 yzk shiyong
            pcl::copyPointCloud(*thisKeyFrame, *thisKeyFrameDS);
            // 2024.04.22 yzk shiyong
...
void pubMap(void)
{
    ...
    pcl::copyPointCloud(*laserCloudMapPGO, *laserCloudMapPGO);
...
void recordSinglePose(void)
{
    std::cout << "recordSinglePose........     keyframe size : " << keyframePosesUpdated.size() << std::endl;
    std::ofstream foutC(output_path + "/final_pose.txt", std::ios::app);

    foutC.setf(std::ios::scientific, std::ios::floatfield);
    foutC.precision(6);

    for (int j = 1; j < keyframePosesUpdated.size(); j++)
    {
        auto cur_pose6d = keyframePosesUpdated[j];
        auto last_pose6d = keyframePosesUpdated[j - 1];

        double cur_time = keyframeTimes[j];
        double last_time = keyframeTimes[j - 1];

        tf::Quaternion q;
        q.setRPY(cur_pose6d.roll, cur_pose6d.pitch, cur_pose6d.yaw);
        Eigen::Vector3d t = Eigen::Vector3d(cur_pose6d.x, cur_pose6d.y, cur_pose6d.z);

        tf::Quaternion q_;
        q_.setRPY(last_pose6d.roll, last_pose6d.pitch, last_pose6d.yaw);
        Eigen::Vector3d t_ = Eigen::Vector3d(last_pose6d.x, last_pose6d.y, last_pose6d.z);

        foutC << std::fixed << cur_time << " ";
        foutC << t.x() << " " << t.y() << " " << t.z() << " ";
        foutC << q[0] << " " << q[1] << " " << q[2] << " " << q[3];
        foutC << std::endl;
    }
